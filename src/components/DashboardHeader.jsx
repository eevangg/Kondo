...
import { DollarSign, Calendar, ShoppingBag, AlertTriangle, Plus, CheckCircle2, ArrowUpRight } from 'lucide-react';
...
export default function DashboardHeader({
  ...
  // 1. Calculate Net Balances between Alex (r1) and Sam (r2)
  ...
  expenses.forEach((exp) => {
    ...
    if (exp.paid_by === r1.id) {
      r1PaidTotal += amt / 2; // half owed by r2
    } else if (exp.paid_by === r2.id) {
      r2PaidTotal += amt / 2; // half owed by r1
    }
  });
  ...
  const netBalance = r1PaidTotal - r2PaidTotal; // Positive = r2 owes r1, Negative = r1 owes r2
  ...
  // 4. Urgent repairs count
  const urgentRepairsCount = maintenanceIssues.filter((m) => m.status !== 'Fixed' && m.priority === 'Urgent').length;
    ...
    <section style={{ marginBottom: '2rem' }}>
      ...
      <div className="grid-cols-4" style={{ marginBottom: '1.25rem' }}>
        ...
        <div className="glass-card glass-card-interactive" style={{ padding: '1.25rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
            <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)', fontWeight: 600 }}>SPLITWISE BALANCE</span>
          ...
          <div>
            {Math.abs(netBalance) < 0.01 ? (
              <div>
                <h3 style={{ fontSize: '1.3rem', color: 'var(--status-success)', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                  <CheckCircle2 size={18} /> All Settled Up
                </h3>
                <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>$0.00 net balance</p>
              </div>
            ) : netBalance > 0 ? (
              <div>
                <h3 style={{ fontSize: '1.3rem', color: '#60a5fa' }}>
                  {r2.name} owes {r1.name}
                </h3>
                <p style={{ fontSize: '1.2rem', fontWeight: 800, color: 'var(--status-success)' }}>
                  ${Math.abs(netBalance).toFixed(2)}
                </p>
              </div>
            ) : (
              <div>
                <h3 style={{ fontSize: '1.3rem', color: '#f472b6' }}>
                  {r1.name} owes {r2.name}
                </h3>
                <p style={{ fontSize: '1.2rem', fontWeight: 800, color: 'var(--status-warning)' }}>
                  ${Math.abs(netBalance).toFixed(2)}
                </p>
              </div>
            )}
        ...
        <div className="glass-card glass-card-interactive" style={{ padding: '1.25rem' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
              {lowStockCount > 0 ? 'Low or out of stock in kitchen' : 'Pantry fully stocked'}
            </p>
          </div>
        </div>
        ...
        <div className="glass-card glass-card-interactive" style={{ padding: '1.25rem' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
              {urgentRepairsCount > 0 ? 'Requires immediate attention' : 'All home repairs clear'}
            </p>
          </div>
        </div>
      ...
      <div className="glass-card" style={{ padding: '0.85rem 1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.75rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
          <button className="btn btn-primary btn-sm" onClick={() => onOpenModal('expense')}>
          ...
          <button className="btn btn-secondary btn-sm" onClick={() => onOpenModal('cleaning')}>
            <Plus size={14} /> Add Cleaning Task
          </button>
