...
export async function sendTelegramMessage(text) {
  ...
  if (!isConfigured) {
    // Fallback: Copy to clipboard if bot credentials aren't saved
    try {
      await navigator.clipboard.writeText(text);
      alert('Telegram Bot not configured yet. Formatted text copied to clipboard! Configure Bot Token in settings for 1-click auto-sending.');
    } catch (e) {
